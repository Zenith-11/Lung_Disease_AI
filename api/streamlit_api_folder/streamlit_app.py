...
def load_pneumonia_model():
    ...
    possible_paths = [
        os.path.join(current_dir, 'best_chest_xray_model.h5'),
...
def is_valid_xray(image_input):
    """
    Heuristic check to determine if the uploaded image is likely an X-ray.
    Rejects highly colorful images (like photos of people or cars).
    """
    if isinstance(image_input, str):
        image = PILImage.open(image_input)
    else:
    ...
    # Ensure RGB mode for array analysis
    if image.mode != 'RGB':
        image = image.convert('RGB')
        
    img_array = np.array(image).astype(np.float32)
    
    # Check variance between color channels
    # X-rays are grayscale, so R, G, B channels should be nearly identical.
    if len(img_array.shape) == 3 and img_array.shape[2] == 3:
        ...
        # If the average difference across channels is significant, it's a color image
        if max(diff_rg, diff_rb, diff_gb) > 10.0:
            return False, "Invalid Image: The uploaded file appears to be a color photograph, not a valid Chest X-ray."
            
    # Check if the image lacks contrast (e.g., completely black, white, or uniform)
    if np.std(img_array) < 5.0:
        return False, "Invalid Image: The uploaded file lacks the typical contrast and structure of a Chest X-ray."
...
def predict_pneumonia(image_input, model=None):
    ...
    """
    try:
        # Perform image validation check first
        is_valid, err_msg = is_valid_xray(image_input)
        if not is_valid:
...
def show_terms_conditions():
    ...
    st.markdown("""
    ### Service Description
    Lung Disease Detect AI provides AI-powered chest X-ray analysis for preliminary pneumonia screening.
...
def show_refund_policy():
    ...
    **Lung Disease Detect AI is currently offered as a free service for research and educational purposes.**
...
def show_contact_us():
    ...
    st.info("🚀 **We're here to help!** Feel free to reach out with any questions about Lung Disease Detect AI.")
...
def generate_medical_pdf_report(prediction_result, analysis_time, original_image=None, ai_focus_image=None):
    ...
    pdf.cell(0, 12, 'Lung Disease Detect AI - Medical Analysis Report', 0, 1, 'C')
    pdf.ln(5)
...
st.markdown(
    ...
    <div class="fixed-header">
        <a href="#" class="header-brand">
            <span class="header-text">Lung Disease Detect AI</span>
        </a>
    </div>
    """,
    unsafe_allow_html=True,
...
st.markdown(
    ...
    <div class="hero">
        ...
        <div class="hero-title">Lung Disease Detect AI</div>
        <div class="hero-tagline">Clinical Grade AI for Rapid Lung Disease Detection</div>
                    ...
                    try:
                        with st.spinner("Generating Enhanced PDF..."):
                            ...
                            if original_img is None:
                                ...
                                filename = f"Lung Disease Detect AI_Enhanced_Report_{int(time.time())}.pdf"
...
st.markdown(
    ...
    <div class="about-developer">
        ...
        <div class="developer-bio">
          Team Sigmoids | Bridging AI & healthcare
        </div>
    </div>
    """,
    unsafe_allow_html=True,
...
st.markdown(
    ...
    <div class="footer">
        ...
        <div>© 2025 {MODEL_SPECS['name']} {MODEL_SPECS['version']} | Team Sigmoids | All Rights Reserved</div>
    </div>
    """,
    unsafe_allow_html=True,
)
